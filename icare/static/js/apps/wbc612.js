$(function(){
    ...
    wbc612.ajax = {
        ...
        get_total_by_vid: function (vid, start, end, cb) {
            app.ajax('/wbc612/get_total_by_vid', {
                start_date: start,
                end_date: end,
                vid: vid
            }, function(e, v) {
        ...
        get_list_by_vid: function (vid, start_date, end_date, s, t, cb) {
            app.ajax('/wbc612/get_list_by_vid', {
                start_date: start_date,
                end_date: end_date,
                vid: vid,
                start: s,
        ...
        get_nutrition_owner: function (pid, hospcode, cb) {
            app.ajax('/wbc612/get_nutrition_owner', {
                pid: pid,
                hospcode: hospcode
            }, function (e, v) {
               e ? cb (e, null) : cb (null, v.rows);
    ...
    wbc612.get_list = function(start, end) {
        wbc612.ajax.get_total(start, end, function(e, total) {
            if (e) {
                app.alert(e);
            } else {
                $('#paging').fadeIn('slow');
                ...
                $('#paging').paging(total, {
                    ...
                    lapping: 1,
                    page: app.get_cookie('wbc612_paging'),
                    onSelect: function(page) {
                        app.set_cookie('wbc612_paging', page);

                        wbc612.ajax.get_list(start, end, this.slice[0], this.slice[1], function(e, rs){
    ...
    wbc612.get_list_by_vid = function(vid, start, end) {
        wbc612.ajax.get_total_by_vid(vid, start, end, function(e, total) {
            if (e) {
                app.alert(e);
            } else {
                $('#paging').fadeIn('slow');
                ...
                $('#paging').paging(total, {
                    ...
                    perpage: app.record_per_page,
                    lapping: 1,
                    page: app.get_cookie('wbc612_vid_paging'),
                    onSelect: function(page) {
                        app.set_cookie('wbc612_vid_paging', page);

                        wbc612.ajax.get_list_by_vid(vid, start, end, this.slice[0], this.slice[1], function(e, rs){
            ...
            $.each(rs, function(i, v) {
                ...
                $('#tbl_list > tbody').append(
                    '<tr>' +
                        ...
                        '<td>' + v.fullname + '</td>' +
                        '<td class="visible-lg">' + v.birth + '</td>' +
    ...
    $(document).on('click', 'a[data-name="btn_get_vaccines"]', function(e) {
        ...
        wbc612.ajax.get_vaccines(pid, function(e, v) {
            if(e) {
                app.alert(e);
            } else {
                wbc612.set_vaccine_list(v);
                wbc612.modal.show_vaccines();
            }
        });
    });

    wbc612.set_vaccine_list = function(v) {
    ...
    $('#btn_filter').on('click', function(e) {
        ...
        if(!start_date) {
            ...
            if(!vid) {
                wbc612.get_list(start_date, end_date);
            } else {
                wbc612.get_list_by_vid(vid, start_date, end_date);
            }
        }
    });
    ...
    $('#btn_search_visit').on('click', function(e) {
        ...
        if(cid) {
            wbc612.do_search_visit(cid);
        } else {
            app.alert('กรุณาระบุเลขบัตรประชาชนเพื่อค้นหา');
        }
    });

    wbc612.do_search_visit = function(cid) {
        wbc612.ajax.search_visit(cid, function(e, v) {
            ...
            if(e) {
                ...
                wbc612.set_visit_history(v);
            }
        });
    };

    wbc612.set_visit_history = function(v) {
        if($(v).size()) {
            $.each(v, function(i, v) {
                i++;
    ...
    $(document).on('click', 'a[data-name="btn_get_nutrition"]', function(e) {
        ...
        wbc612.get_nutrition(cid);
    });

    wbc612.get_nutrition = function(cid) {

        $('#tbl_nutrition > tbody').empty();

        wbc612.ajax.get_nutrition(cid, function(e, v) {
            if(e) {;
                app.alert(e);
            } else {
                wbc612.set_nutrition(v);
                wbc612.modal.show_nutrition();
            }
        });
    };
    wbc612.get_nutrition_owner = function(pid, hospcode) {

        $('#tbl_nutrition > tbody').empty();

        wbc612.ajax.get_nutrition_owner(pid, hospcode, function(e, v) {
            if(e) {;
                app.alert(e);
            } else {
                wbc612.set_nutrition(v);
    ...
    wbc612.set_nutrition = function(v) {
        if($(v).size()) {
            $.each(v, function(i, v) {
                i++;
    ...
    $(document).on('click', 'a[data-name="btn_visit_nutrition" ]', function(e) {
        ...
        wbc612.get_nutrition_owner(pid, hospcode);
    });
    ...
    $(document).on('click', 'a[data-name="btn_get_appointment"]', function(e) {
        ...
        wbc612.ajax.get_appointment(pid, hospcode, seq, function(e, v) {
            if(e) {
                app.alert(e);
            } else {
                wbc612.set_appoint(v);
                wbc612.modal.show_appoint();
            }
        });
    });

    wbc612.set_appoint = function(rs) {
